...
Method `$content->body();` can accept any renderable objects, like string, number, class that has method `__toString()`, or implements `Renderable, `Htmlable` interface, include Laravel view objects.

## Layout

`laravel-admin` use grid system of bootstrap, the length of each row is 12, the following is a few simple examples:
